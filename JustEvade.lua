...
local function AutoUpdate()
    DownloadFile(
        "https://raw.githubusercontent.com/Ark223/GoS-Scripts/master/JustEvade.version",
    ...
    if tonumber(ReadFile(SCRIPT_PATH .. "JustEvade.version")) > Version then
        ...
        DownloadFile(
            "https://raw.githubusercontent.com/Ark223/GoS-Scripts/master/JustEvade.lua",
            SCRIPT_PATH .. "JustEvade.lua")
        print("JustEvade: Successfully updated. Use 2x F6!")
    end
end

local MathAbs, MathAtan, MathAtan2, MathAcos, MathCeil, MathCos, MathDeg,
      MathFloor, MathHuge, MathMax, MathMin, MathPi, MathRad, MathSin, MathSqrt =
    math.abs, math.atan, math.atan2, math.acos, math.ceil, math.cos, math.deg,
    math.floor, math.huge, math.max, math.min, math.pi, math.rad, math.sin,
    math.sqrt
local GameCanUseSpell, GameLatency, GameTimer, GameHeroCount, GameHero,
      GameMinionCount, GameMinion, GameMissileCount, GameMissile =
    Game.CanUseSpell, Game.Latency, Game.Timer, Game.HeroCount, Game.Hero,
    Game.MinionCount, Game.Minion, Game.MissileCount, Game.Missile
local DrawCircle, DrawColor, DrawLine, DrawText, ControlKeyUp, ControlKeyDown,
      ControlMouseEvent, ControlSetCursorPos = Draw.Circle, Draw.Color,
                                               Draw.Line, Draw.Text,
                                               Control.KeyUp, Control.KeyDown,
                                               Control.mouse_event,
                                               Control.SetCursorPos
local TableInsert, TableRemove, TableSort = table.insert, table.remove,
                                            table.sort
local Icons, Png = "https://raw.githubusercontent.com/Ark223/LoL-Icons/master/",
                   ".png"
...
local function IsPoint(p)
    return p and p.x and type(p.x) == "number" and
               (p.y and type(p.y) == "number")
end

local function Round(v) return v < 0 and MathCeil(v - 0.5) or MathFloor(v + 0.5) end
...
function Point2D:__type() return "Point2D" end

function Point2D:__eq(p) return self.x == p.x and self.y == p.y end
...
function Point2D:__tostring() return "(" .. self.x .. ", " .. self.y .. ")" end

function Point2D:Clone() return Point2D(self) end

function Point2D:Extended(to, distance)
    return self + (Point2D(to) - self):Normalized() * distance
end

function Point2D:Magnitude() return MathSqrt(self:MagnitudeSquared()) end
...
function Point2D:Perpendicular() return Point2D(-self.y, self.x) end

function Point2D:Perpendicular2() return Point2D(self.y, -self.x) end
        ...
        if not subject.intersection then
            ...
            while clip.next do
                if not clip.intersection then
                    ...
                    local int, segs = self:Intersection(subject, subjNext, clip,
                                                        clipNext)
                    if int and segs then
                        found = true
                        local alpha1 = self:Distance(subject, int) /
                                           self:Distance(subject, subjNext)
                        local alpha2 = self:Distance(clip, int) /
                                           self:Distance(clip, clipNext)
                        local subjectInter =
                            Vertex:New(int.x, int.y, alpha1, true)
...
function XPolygon:IdentifyIntersectionType(subjList, clipList, clipPoly,
                                           subjPoly, operation)
...
function XPolygon:CrossProduct(p1, p2) return p1.x * p2.y - p1.y * p2.x end

function XPolygon:Distance(p1, p2) return MathSqrt(self:DistanceSquared(p1, p2)) end
...
function XPolygon:Intersection(a1, b1, a2, b2)
    ...
    local r, s = Point2D(b1 - a1), Point2D(b2 - a2);
    local x = self:CrossProduct(r, s)
    local t, u = self:CrossProduct(a2 - a1, s) / x,
                 self:CrossProduct(a2 - a1, r) / x
...
function XPolygon:IsPointInPolygon(poly, point)
    ...
    for i = 1, #poly do
        if poly[i].y < point.y and poly[j].y >= point.y or poly[j].y < point.y and
            poly[i].y >= point.y then
            if poly[i].x + (point.y - poly[i].y) / (poly[j].y - poly[i].y) *
                (poly[j].x - poly[i].x) < point.x then
                result = not result
            end
        end
        j = i
...
function XPolygon:OffsetPolygon(poly, offset)
    ...
    for i, point in ipairs(poly) do
        local j, k = i - 1, i + 1
        if j < 1 then j = #poly end
        if k > #poly then k = 1 end
        ...
        local dot = (p1.x - p2.x) * (p3.x - p2.x) + (p1.y - p2.y) *
                        (p3.y - p2.y)
        local cross = (p1.x - p2.x) * (p3.y - p2.y) - (p1.y - p2.y) *
                          (p3.x - p2.x)
        ...
        if dist > offset and angle > 0 then
            ...
            local i1 = self:Intersection(e, f, a, b);
            local i2 = self:Intersection(e, f, c, d)
            TableInsert(result, i1);
            TableInsert(result, i2)
        else
...
function JEvade:__init()
    self.DoD, self.Evading, self.InsidePath, self.Loaded = false, false, false,
                                                           false
    self.ExtendedPos, self.Flash, self.Flash2, self.MousePos, self.MyHeroPos, self.SafePos =
    ...
    self.Debug, self.DodgeableSpells, self.DetectedSpells, self.Enemies, self.EvadeSpellData, self.OnCreateMisCBs, self.OnImpDodgeCBs, self.OnProcSpellCBs =
        {}, {}, {}, {}, {}, {}, {}, {}
    self.DDTimer, self.DebugTimer, self.MoveTimer, self.MissileID, self.OldTimer, self.NewTimer =
        0, 0, 0, 0, 0, 0
    ...
    TableSort(self.Enemies,
              function(a, b) return a.unit.charName < b.unit.charName end)
    self.JEMenu = MenuElement({
        type = MENU,
        id = "JustEvade",
        name = "Just Evade v" .. IntVer
    })
    self.JEMenu:MenuElement({id = "Core", name = "Core Settings", type = MENU})
    self.JEMenu.Core:MenuElement({
        id = "SmoothEvade",
        name = "Enable Smooth Evading",
        value = true
    })
    self.JEMenu.Core:MenuElement({
        id = "LimitRange",
        name = "Limit Detection Range",
        value = true
    })
    self.JEMenu.Core:MenuElement({
        id = "GP",
        name = "Average Game Ping",
        value = 50,
        min = 0,
        max = 250,
        step = 5
    ...
    self.JEMenu.Main:MenuElement({
        id = "Missile",
        name = "Enable Missile Detection",
        value = false
    })
    self.JEMenu.Main:MenuElement({
        id = "Debug",
        name = "Debug Evade Points",
        value = true
    })
    self.JEMenu.Main:MenuElement({
        id = "Status",
        name = "Draw Evade Status",
        value = true
    })
    self.JEMenu.Main:MenuElement({
        id = "SafePos",
        name = "Draw Safe Position",
        value = true
    })
    self.JEMenu.Main:MenuElement({
        id = "DD",
        name = "Dodge Only Dangerous",
        key = string.byte("N")
    })
    self.JEMenu.Main:MenuElement({
        id = "Arrow",
        name = "Dodge Arrow Color",
        color = DrawColor(192, 255, 255, 0)
    })
    self.JEMenu.Main:MenuElement({
        id = "SPC",
        name = "Safe Position Color",
        color = DrawColor(192, 255, 255, 255)
    })
    self.JEMenu.Main:MenuElement({
        id = "SC",
        name = "Detected Spell Color",
        color = DrawColor(192, 255, 255, 255)
    })
    self.JEMenu:MenuElement({
        id = "Spells",
                ...
                if eS[i] then
                    self.JEMenu.Spells:MenuElement({
                        ...
                        name = "" .. myHero.charName .. " " ..
                            self.SpellSlot[eS[i].slot] .. " - " ..
                            eS[i].displayName,
                        leftIcon = eS[i].icon,
                        type = MENU
                    })
                    self.JEMenu.Spells[eS[i].name]:MenuElement({
                        id = "US" .. eS[i].name,
                        name = "Use Spell",
                        value = true
                    })
    ...
    self.SpecialSpells = {
        ["PantheonR"] = function(sP, eP, data)
            local sP2, eP2 = Point2D(eP):Extended(sP, 1150),
                             self:AppendVector(sP, eP, 200)
            return self:RectangleToPolygon(sP2, eP2, data.radius,
                                           self.BoundingRadius),
                   self:RectangleToPolygon(sP2, eP2, data.radius)
        end,
        ["ZoeE"] = function(sP, eP, data)
            local p1 = self:CircleToPolygon(eP,
                                            data.radius + self.BoundingRadius,
                                            self.JEMenu.Core.CQ:Value())
            local p2 = self:CircleToPolygon(eP, data.radius,
                                            self.JEMenu.Core.CQ:Value())
            self:AddSpell(p1, p2, sP, eP, data, MathHuge, data.range, 5, 250,
                          "ZoeE")
            return p1, p2
        end,
        ["AatroxQ2"] = function(sP, eP, data)
            local dir = Point2D(sP - eP):Perpendicular():Normalized() *
                            data.radius
            local s1, s2 = Point2D(sP - dir), Point2D(sP + dir)
            local e1, e2 = self:Rotate(s1, eP, MathRad(40)),
                           self:Rotate(s2, eP, -MathRad(40))
        ...
        ["GravesQLineSpell"] = function(sP, eP, data)
            ...
            local p1, p2 = self:RectangleToPolygon(sP, eP, data.radius),
                           self:RectangleToPolygon(s1, e1, 150)
            local path = XPolygon:ClipPolygons(p1, p2, "union")
            return XPolygon:OffsetPolygon(path, self.BoundingRadius), path
        end,
        ["GravesChargeShot"] = function(sP, eP, data)
            local p1, e1 = self:RectangleToPolygon(sP, eP, data.radius),
                           self:AppendVector(sP, eP, 700)
            local dir = Point2D(eP - e1):Perpendicular():Normalized() * 350
            local path = {
                p1[2], p1[3], Point2D(e1 - dir), Point2D(e1 + dir), p1[4], p1[1]
            }
            return XPolygon:OffsetPolygon(path, self.BoundingRadius), path
        end,
        ...
        ["MordekaiserQ"] = function(sP, eP, data)
            ...
            local s1, s2 = Point2D(sP - dir), Point2D(sP + dir)
            local e1 = self:Rotate(s1, Point2D(s1):Extended(eP, 675),
                                   -MathRad(18))
            local e2 = self:Rotate(s2, Point2D(s2):Extended(eP, 675),
                                   MathRad(18))
            local path = {s1, e1, e2, s2}
            return XPolygon:OffsetPolygon(path, self.BoundingRadius), path
        end,
        ...
        ["OrianaIzuna"] = function(sP, eP, data)
            ...
            local p2 =
                self:CircleToPolygon(eP, 135, self.JEMenu.Core.CQ:Value())
            local path = XPolygon:ClipPolygons(p1, p2, "union")
            return XPolygon:OffsetPolygon(path, self.BoundingRadius), path
        end,
        ["SettW"] = function(sP, eP, data)
            ...
            local dir = Point2D(ePos - sPos):Perpendicular():Normalized() *
                            data.radius
            local s1, s2 = Point2D(sPos - dir), Point2D(sPos + dir)
            local e1 = self:Rotate(s1, Point2D(s1):Extended(ePos, data.range),
                                   -MathRad(30))
            local e2 = self:Rotate(s2, Point2D(s2):Extended(ePos, data.range),
                                   MathRad(30))
            local path = {s1, e1, e2, s2}
            return XPolygon:OffsetPolygon(path, self.BoundingRadius), path
        end,
        ["SettE"] = function(sP, eP, data)
            ...
            return self:RectangleToPolygon(sPos, eP, data.radius,
                                           self.BoundingRadius),
        ...
        ["SylasQ"] = function(sP, eP, data)
            ...
            local s1, s2 = Point2D(sP - dir), Point2D(sP + dir)
            local e1 = self:Rotate(s1, Point2D(s1):Extended(eP, data.range),
                                   MathRad(3))
            local e2 = self:Rotate(s2, Point2D(s2):Extended(eP, data.range),
                                   -MathRad(3))
            local p1, p2 = self:RectangleToPolygon(s1, e1, data.radius),
                           self:RectangleToPolygon(s2, e2, data.radius)
            local p3 =
                self:CircleToPolygon(eP, 180, self.JEMenu.Core.CQ:Value())
            local path = XPolygon:ClipPolygons(p1, p2, "union")
            return XPolygon:OffsetPolygon(path, self.BoundingRadius), path
        end,
        ...
        ["ZiggsQ"] = function(sP, eP, data)
            ...
            local p1, bp1 = self:CircleToPolygon(eP, data.radius, quality),
                            self:CircleToPolygon(eP, data.radius +
                                                     self.BoundingRadius,
                                                 quality)
            local e1 = Point2D(sP):Extended(eP, 1.4 * self:Distance(sP, eP))
            local p2, bp2 = self:CircleToPolygon(e1, data.radius, quality),
                            self:CircleToPolygon(e1, data.radius +
                                                     self.BoundingRadius,
                                                 quality)
            local e2 = Point2D(eP):Extended(e1, 1.69 * self:Distance(eP, e1))
            local p3, bp3 = self:CircleToPolygon(e2, data.radius, quality),
                            self:CircleToPolygon(e2, data.radius +
                                                     self.BoundingRadius,
                                                 quality)
            self:AddSpell(bp1, p1, sP, eP, data, data.speed, data.range, 0.25,
                          data.radius, "ZiggsQ")
            self:AddSpell(bp2, p2, sP, eP, data, data.speed, data.range, 0.75,
                          data.radius, "ZiggsQ")
            self:AddSpell(bp3, p3, sP, eP, data, data.speed, data.range, 1.25,
                          data.radius, "ZiggsQ")
            return nil, nil
        end
    }
    self.SpellTypes = {
        ["linear"] = function(sP, eP, data)
            return self:RectangleToPolygon(sP, eP, data.radius,
                                           self.BoundingRadius),
                   self:RectangleToPolygon(sP, eP, data.radius)
        end,
        ...
        ["rectangular"] = function(sP, eP, data)
            local dir = Point2D(eP - sP):Perpendicular():Normalized() *
                            (data.radius2 or 400)
            local sP2, eP2 = Point2D(eP - dir), Point2D(eP + dir)
            return self:RectangleToPolygon(sP2, eP2, data.radius / 2,
                                           self.BoundingRadius),
                   self:RectangleToPolygon(sP2, eP2, data.radius / 2)
        end,
        ["circular"] = function(sP, eP, data)
            local quality = self.JEMenu.Core.CQ:Value()
            return self:CircleToPolygon(eP, data.radius + self.BoundingRadius,
                                        quality),
    ...
    DelayAction(function()
        ...
        if self.Flash then
            ...
            self.JEMenu.Spells.Flash:MenuElement({
                id = "US",
                name = "Use Flash",
                value = true
            })
...
function JEvade:DrawArrow(startPos, endPos, color)
    local p1 = endPos -
                   (Point2D(startPos - endPos):Normalized() * 30):Perpendicular() +
                   Point2D(startPos - endPos):Normalized() * 30
    local p2 = endPos -
                   (Point2D(startPos - endPos):Normalized() * 30):Perpendicular2() +
                   Point2D(startPos - endPos):Normalized() * 30
    local startPos, endPos, p1, p2 = self:FixPos(startPos), self:FixPos(endPos),
                                     self:FixPos(p1), self:FixPos(p2)
...
function JEvade:DrawPolygon(poly, y, color)
    ...
    for i = 1, #path - 1 do
        DrawLine(path[i].x, path[i].y, path[i + 1].x, path[i + 1].y, 0.5, color)
    end
end
...
function JEvade:CalculateEndPos(startPos, placementPos, unitPos, speed, range,
                                radius, collision, type, extend)
    ...
    if not extend then
        ...
        if type == "linear" then
            if speed ~= MathHuge then
                endPos = self:AppendVector(startPos, endPos, radius)
            end
            if collision then
                local startPos, minions =
                    Point2D(startPos):Extended(placementPos, 45), {}
                for i = 1, GameMinionCount() do
                    local minion = GameMinion(i);
                    local minionPos = self:To2D(minion.pos)
                    if minion and minion.team == myHero.team and minion.valid and
                        Minions[minion.charName] and
                        self:Distance(minionPos, startPos) <= range and
                        minion.maxHealth > 295 and minion.health > 5 then
                        local col = self:ClosestPointOnSegment(startPos,
                                                               placementPos,
                                                               minionPos)
                        if col and self:Distance(col, minionPos) <
                            ((minion.boundingRadius or 45) / 2 + radius) then
                ...
                if #minions > 0 then
                    TableSort(minions, function(a, b)
                        return self:DistanceSquared(a, startPos) <
                                   self:DistanceSquared(b, startPos)
                    end)
                    local range2 = self:Distance(startPos, minions[1])
                    local endPos = Point2D(startPos):Extended(placementPos,
                                                              range2)
...
function JEvade:CircleToPolygon(pos, radius, quality)
    ...
    for i = 0, (quality or 16) - 1 do
        ...
        local cx, cy = pos.x + radius * MathCos(angle),
                       pos.y + radius * MathSin(angle)
...
function JEvade:ClosestPointOnSegment(s1, s2, pt)
    ...
    local t = ((pt.x - s1.x) * ab.x + (pt.y - s1.y) * ab.y) /
                  (ab.x * ab.x + ab.y * ab.y)
    return t < 0 and Point2D(s1) or
               (t > 1 and Point2D(s2) or Point2D(s1 + t * ab))
end
...
function JEvade:CrossProduct(p1, p2) return p1.x * p2.y - p1.y * p2.x end

function JEvade:Distance(p1, p2) return MathSqrt(self:DistanceSquared(p1, p2)) end

function JEvade:DistanceSquared(p1, p2)
    return (p2.x - p1.x) ^ 2 + (p2.y - p1.y) ^ 2
end

function JEvade:DotProduct(p1, p2) return p1.x * p2.x + p1.y * p2.y end
...
function JEvade:GetBestEvadePos(spells, radius, mode, extra, force)
    local evadeModes = {
        [1] = function(a, b)
            return self:DistanceSquared(a, self.MyHeroPos) <
                       self:DistanceSquared(b, self.MyHeroPos)
        end,
        [2] = function(a, b)
            local mPos = self.MyHeroPos:Extended(self.MousePos,
    ...
    for i, spell in ipairs(spells) do
        ...
        for j = 1, #poly do
            ...
            local original = self:ClosestPointOnSegment(startPos, endPos,
                                                        self.MyHeroPos)
            local distSqr = self:DistanceSquared(original, self.MyHeroPos)
            if distSqr <= 360000 then
                if force then
                    local candidate = self:AppendVector(self.MyHeroPos,
                                                        original, 5)
                    if distSqr <= 160000 and not self:IsDangerous(candidate) and
                        not MapPosition:inWall(self:To3D(candidate)) then
                    ...
                    for k = -step, step, 1 do
                        local candidate =
                            Point2D(original + k * self.JEMenu.Core.DS:Value() *
                                        direction)
                        local extended =
                            self:AppendVector(self.MyHeroPos, candidate,
                                              self.BoundingRadius)
                        candidate = self:AppendVector(self.MyHeroPos, candidate,
                                                      5)
                        if self:IsSafePos(candidate, extra) and
                            not MapPosition:inWall(self:To3D(extended)) then
                            TableInsert(points, candidate)
                        end
                    end
                end
...
function JEvade:GetExtendedSafePos(pos)
    ...
    for i = 2, 8 do
        ...
        local ext = self:AppendVector(self.MyHeroPos, pos,
                                      self.BoundingRadius * i)
...
function JEvade:GetMovePath()
    return self:IsMoving() and myHero.pathing.endPos ~= nil and
               self:To2D(myHero.pathing.endPos) or nil
end
...
function JEvade:IsAboutToHit(spell, pos, extra)
    local evadeSpell = #self.EvadeSpellData > 0 and
                           self.EvadeSpellData[extra or 1] or nil
    ...
    local diff, pos = GameTimer() - spell.startTime,
                      self:AppendVector(myPos, pos, 99999)
    if spell.speed ~= MathHuge and spell.type == "linear" or spell.type ==
        "threeway" then
        if spell.delay > 0 and diff <= spell.delay then
            myPos = Point2D(myPos):Extended(pos,
                                            (spell.delay - diff) * moveSpeed)
            if not self:IsPointInPolygon(spell.path, myPos) then
                return false
            end
        end
        local va = Point2D(pos - myPos):Normalized() * moveSpeed
        local vb = Point2D(spell.endPos - spell.position):Normalized() *
                       spell.speed
        local da, db = Point2D(myPos - spell.position), Point2D(va - vb)
        local a, b = self:DotProduct(db, db), 2 * self:DotProduct(da, db)
        local c = self:DotProduct(da, da) -
                      (spell.radius + self.BoundingRadius * 2) ^ 2
...
function JEvade:IsSafePos(pos, extra)
    for i, s in ipairs(self.DodgeableSpells) do
        if self:IsPointInPolygon(s.path, pos) or
            self:IsAboutToHit(s, pos, extra) then return false end
    end
    return true
end
...
function JEvade:LineSegmentIntersection(a1, b1, a2, b2)
    ...
    local t, u = self:CrossProduct(a2 - a1, s) / x,
                 self:CrossProduct(a2 - a1, r) / x
    return x ~= 0 and t >= 0 and t <= 1 and u >= 0 and u <= 1 and
               Point2D(a1 + t * r) or nil
end

function JEvade:Magnitude(p) return MathSqrt(self:MagnitudeSquared(p)) end

function JEvade:MagnitudeSquared(p) return p.x * p.x + p.y * p.y end
...
function JEvade:RectangleToPolygon(startPos, endPos, radius, offset)
    ...
    return {
        Point2D(startPos + perp - offset * dir),
        Point2D(startPos - perp - offset * dir),
        Point2D(endPos - perp + offset * dir),
        Point2D(endPos + perp + offset * dir)
    }
end
...
function JEvade:Rotate(startPos, endPos, theta)
    ...
    local px, py = dx * MathCos(theta) - dy * MathSin(theta),
                   dx * MathSin(theta) + dy * MathCos(theta)
...
function JEvade:To2D(pos) return Point2D(pos.x, pos.z or pos.y) end

function JEvade:To3D(pos) return Vector(pos.x, myHero.pos.y, pos.y) end
...
function JEvade:CreateMissile(func) TableInsert(self.OnCreateMisCBs, func) end

function JEvade:GetDodgeableSpells()
    local paths, result = {}, {}
    for i, s in ipairs(self.DetectedSpells) do
        self:SpellManager(i, s)
        if self.JEMenu.Main.Dodge:Value() and
            self.JEMenu.Spells[s.name]["Dodge" .. s.name]:Value() and
            self:GetHealthPercent() <=
            self.JEMenu.Spells[s.name]["HP" .. s.name]:Value() then
            if self.DoD and s.danger >= 4 or not self.DoD then
...
function JEvade:GetHealthPercent() return myHero.health / myHero.maxHealth * 100 end

function JEvade:GetMovementSpeed(extra, evadeSpell)
    local moveSpeed = myHero.ms or 315
    if not extra then return moveSpeed end
    if not evadeSpell then return 9999 end
    local lvl, name = myHero:GetSpellData(evadeSpell.slot).level or 1,
                      evadeSpell.name
    if lvl == nil or lvl == 0 then return moveSpeed end
    if name == "AnnieE-" then
        return (1.2824 + (0.0176 * myHero.levelData.lvl)) * moveSpeed
    elseif name == "BlitzcrankW-" then
        return ({1.7, 1.75, 1.8, 1.85, 1.9})[lvl] * moveSpeed
    elseif name == "DravenW-" then
        return ({1.4, 1.45, 1.5, 1.55, 1.6})[lvl] * moveSpeed
    elseif name == "GarenQ-" then
        return 1.3 * moveSpeed
    elseif name == "KaisaE-" then
        return ({1.55, 1.6, 1.65, 1.7, 1.75})[lvl] * 2 *
                   MathMin(1, myHero.attackSpeed) * moveSpeed
    elseif name == "KatarinaW-" then
        return ({1.5, 1.6, 1.7, 1.8, 1.9})[lvl] * moveSpeed
    elseif name == "KennenE-" then
        return 2 * moveSpeed
    elseif name == "RumbleW-" then
        return ({1.2, 1.25, 1.3, 1.35, 1.4})[lvl] * moveSpeed
    elseif name == "ShyvanaW-" then
        return ({1.3, 1.35, 1.4, 1.45, 1.5})[lvl] * moveSpeed
    elseif name == "SkarnerW-" then
        return ({1.08, 1.1, 1.12, 1.14, 1.16})[lvl] * moveSpeed
    elseif name == "SonaE-" then
        return (({1.1, 1.11, 1.12, 1.13, 1.14})[lvl] + myHero.ap / 100 * 0.03) *
                   moveSpeed
    elseif name == "TeemoW-" then
        return ({1.1, 1.14, 1.18, 1.22, 1.26})[lvl] * moveSpeed
    elseif name == "UdyrE-" then
        return ({1.15, 1.2, 1.25, 1.3, 1.35, 1.4})[lvl] * moveSpeed
    elseif name == "VolibearQ-" then
        return ({1.15, 1.175, 1.2, 1.225, 1.25})[lvl] * moveSpeed
    end
    return moveSpeed
end
...
function JEvade:ImpossibleDodge(func) TableInsert(self.OnImpDodgeCBs, func) end

function JEvade:IsMoving() return myHero.pos.x - MathFloor(myHero.pos.x) ~= 0 end

function JEvade:IsReady(spell) return GameCanUseSpell(spell) == 0 end
...
function JEvade:ProcessSpell(func) TableInsert(self.OnProcSpellCBs, func) end
...
function JEvade:ValidTarget(target, range)
    ...
    return target and target.valid and target.visible and not target.dead and
               self:DistanceSquared(self.MyHeroPos, self:To2D(target.pos)) <=
               range * range
end
...
function JEvade:LoadEvadeSpells()
    if myHero:GetSpellData(SUMMONER_1).name == "SummonerFlash" then
        self.Flash, self.Flash2 = HK_SUMMONER_1, SUMMONER_1
    elseif myHero:GetSpellData(SUMMONER_2).name == "SummonerFlash" then
        self.Flash, self.Flash2 = HK_SUMMONER_2, SUMMONER_2
    end
    for i = 0, 3 do
        local eS = EvadeSpells[myHero.charName]
        if eS and eS[i] then
...
function JEvade:Tick()
    ...
    for i = 1, #self.Enemies do
        ...
        if unit and unit.valid and not unit.dead then
            ...
            if active and active.valid and spell ~= active.name ..
                active.endTime and active.isChanneling then
                self.Enemies[i].spell = active.name .. active.endTime
                self:OnProcessSpell(unit, active)
                for i = 1, #self.OnProcSpellCBs do
                    self.OnProcSpellCBs[i](unit, active)
                end
            end
        end
    end
        ...
        if movePath and not self.Evading then
            ...
            for i, s in ipairs(self.DodgeableSpells) do
                ...
                if not self:IsPointInPolygon(poly, self.MyHeroPos) then
                    local findInts = self:FindIntersections(poly,
                                                            self.MyHeroPos,
                                                            movePath)
                    for i, int in ipairs(findInts) do
                        TableInsert(ints, int)
                    end
                end
            end
            if #ints > 0 then
                TableSort(ints, function(a, b)
                    return self:DistanceSquared(self.MyHeroPos, a) <
                               self:DistanceSquared(self.MyHeroPos, b)
                end)
                local movePos = self:PrependVector(self.MyHeroPos, ints[1],
                                                   self.BoundingRadius / 2)
...
function JEvade:CoreManager(s)
    if self:IsPointInPolygon(s.path, self.MyHeroPos) then
        if self.OldTimer ~= self.NewTimer then
            ...
            local flashUsage = self.Flash2 and
                                   self.JEMenu.Spells.Flash.US:Value() and
                                   self:IsReady(self.Flash2) and s.danger == 5
            local safePos = self:GetBestEvadePos(self.DodgeableSpells, s.radius,
                                                 2, nil, false)
            if safePos then
            ...
            elseif evadeSpells and #evadeSpells > 0 or flashUsage then
                ...
                for i = 1, #evadeSpells do
                    local alternPos = self:GetBestEvadePos(self.DodgeableSpells,
                                                           s.radius, 1, i, false)
                    result = self:Avoid(s, alternPos, evadeSpells[i])
                    if result > 0 then
                        if result == 1 then
                            self.ExtendedPos =
                                self:GetExtendedSafePos(alternPos)
                ...
                if result == 0 then
                    local dodgePos = self:GetBestEvadePos(self.DodgeableSpells,
                                                          s.radius, 1, true,
                                                          true)
                    if dodgePos then
                        if flashUsage then
                            result = 1;
                            self:CastSpell(self.Flash, self:To3D(dodgePos))
                ...
                if result == 0 then
                    for i = 1, #self.OnImpDodgeCBs do
                        self.OnImpDodgeCBs[i](s.danger)
                    end
                end
            else
                for i = 1, #self.OnImpDodgeCBs do
    ...
    if s.startTime + s.range / s.speed + s.delay > GameTimer() then
        if s.speed ~= MathHuge and s.startTime + s.delay < GameTimer() then
            if s.type == "linear" or s.type == "threeway" then
                ...
                s.position = sP
                s.path = self:RectangleToPolygon(sP, s.endPos, s.radius,
                                                 self.BoundingRadius)
...
function JEvade:Avoid(spell, dodgePos, data)
    if self:IsReady(data.slot) and
        self.JEMenu.Spells[data.name]["US" .. data.name]:Value() and
        spell.danger >=
        self.JEMenu.Spells[data.name]["Danger" .. data.name]:Value() then
        if dodgePos and (data.type == 1 or data.type == 2) then
            if data.type == 1 then
                local dashPos = Point2D(self.MyHeroPos):Extended(dodgePos,
                                                                 data.range)
                _G.Control.CastSpell(data.slot2, self:To3D(dashPos));
                return 1
            elseif data.type == 2 then
                _G.Control.CastSpell(data.slot2, myHero.pos);
                return 1
            end
        ...
        elseif data.type == 4 then
            for i = 1, GameHeroCount() do
                ...
                if enemy and self:ValidTarget(enemy, data.range) and myHero.team ~=
                    enemy.team then
                    _G.Control.CastSpell(data.slot2, enemy.pos);
                    return 2
                end
            end
        ...
        elseif data.type == 6 and spell.windwall then
            local wallPos, mPos = Point2D(self.MyHeroPos):Extended(
                                      spell.position, 100), mousePos
            if _G.SDK then
            ...
            DelayAction(function()
                ...
                ControlKeyDown(data.slot2);
                ControlKeyUp(data.slot2)
                DelayAction(function()
        ...
        elseif data.type == 7 and spell.cc then
            _G.Control.CastSpell(data.slot2, self:To3D(spell.position));
            return 2
        end
    end
    return 0
...
function JEvade:Draw()
    ...
    if self.JEMenu.Main.Status:Value() then
        if self.JEMenu.Main.Evade:Value() then
            if self.DoD then
                self:DrawText("Evade: Dodge Only Dangerous", 14, myHero.pos2D,
                              -83, 45, DrawColor(224, 255, 255, 0))
            else
                self:DrawText("Evade: ON", 14, myHero.pos2D, -30, 45,
                              DrawColor(224, 255, 255, 255))
            end
        else
            self:DrawText("Evade: OFF", 14, myHero.pos2D, -32, 45,
                          DrawColor(224, 255, 255, 255))
        end
    end
    if #self.DetectedSpells > 0 and self.Evading and self.SafePos ~= nil and
        self.JEMenu.Main.SafePos:Value() then
        DrawCircle(self:To3D(self.SafePos), self.BoundingRadius, 0.5,
                   self.JEMenu.Main.SPC:Value())
        self:DrawArrow(self.MyHeroPos, self.SafePos,
                       self.JEMenu.Main.Arrow:Value())
    end
    if self.JEMenu.Main.Draw:Value() then
        if self.JEMenu.Main.Debug:Value() then
            for i, dbg in ipairs(self.Debug) do
                DrawCircle(self:To3D(dbg), self.BoundingRadius, 0.5,
                           DrawColor(192, 255, 255, 0))
            end
        end
        for i, s in ipairs(self.DetectedSpells) do
...
function JEvade:OnProcessSpell(unit, spell)
    if unit and spell then
        if unit.team ~= myHero.team then
            ...
            if self.JEMenu.Core.LimitRange:Value() and
                self:Distance(self.MyHeroPos, unitPos) >
                self.JEMenu.Core.LR:Value() then return end
            if SpellDatabase[unit.charName] and
                SpellDatabase[unit.charName][name] then
                local data = self:CopyTable(SpellDatabase[unit.charName][name])
                if data.exception then return end
                local startPos, placementPos = self:To2D(spell.startPos),
                                               self:To2D(spell.placementPos)
                local endPos, range = self:CalculateEndPos(startPos,
                                                           placementPos,
                                                           unitPos, data.speed,
                                                           data.range,
                                                           data.radius,
                ...
                data.range, data.radius, data.y = range, data.radius +
                                                      (self.JEMenu.Spells[name]["ER" ..
                                                          name]:Value() or 0),
                                                  spell.placementPos.y
                ...
                self:AddSpell(path, path2, startPos, endPos, data, data.speed,
                              range, data.delay, data.radius, name)
                if data.type == "threeway" then
                    for i = 1, 2 do
                        local eP = i == 1 and
                                       self:Rotate(startPos, endPos,
                                                   MathRad(data.angle)) or
                                       self:Rotate(startPos, endPos,
                                                   -MathRad(data.angle))
                        local p1 = self:RectangleToPolygon(startPos, eP,
                                                           data.radius,
                                                           self.BoundingRadius)
                        local p2 = self:RectangleToPolygon(startPos, eP,
                                                           data.radius)
                        self:AddSpell(p1, p2, startPos, eP, data, data.speed,
                                      range, data.delay, data.radius, name)
                    end
                end
        ...
        elseif unit == myHero and spell.name == "SummonerFlash" then
            self.NewTimer, self.SafePos, self.ExtendedPos = GameTimer(), nil,
                                                            nil
        end
    end
end
...
function JEvade:OnCreateMissile(unit, missile)
    ...
    if string.find(name, "ttack") or not SpellDatabase[unit.charName] then
        return
    end
    if self.JEMenu.Core.LimitRange:Value() and
        self:Distance(self.MyHeroPos, unitPos) > self.JEMenu.Core.LR:Value() then
        return
    end
    local menuName = ""
    ...
    if self.JEMenu.Spells[menuName]["FOW" .. menuName]:Value() and
        not unit.visible and not data.exception or
        (data.exception and unit.visible) then
        local startPos, placementPos = self:To2D(missile.startPos),
                                       self:To2D(missile.endPos)
        local endPos, range = self:CalculateEndPos(startPos, placementPos,
                                                   unitPos, data.speed,
                                                   data.range, data.radius,
                                                   data.collision, data.type,
                                                   data.extend)
        data.range, data.radius, data.y = range, data.radius +
                                              (self.JEMenu.Spells[menuName]["ER" ..
                                                  menuName]:Value() or 0),
                                          missile.endPos.y
        local path, path2 = self:GetPaths(startPos, endPos, data, name)
        if path == nil then return end
        ...
        self:AddSpell(path, path2, startPos, endPos, data, data.speed, range, 0,
                      data.radius, menuName)
        if data.type == "threeway" then
            for i = 1, 2 do
                local eP = i == 1 and
                               self:Rotate(startPos, endPos, MathRad(data.angle)) or
                               self:Rotate(startPos, endPos,
                                           -MathRad(data.angle))
                local p1 = self:RectangleToPolygon(startPos, eP, data.radius,
                                                   self.BoundingRadius)
                local p2 = self:RectangleToPolygon(startPos, eP, data.radius)
                self:AddSpell(p1, p2, startPos, eP, data, data.speed, range, 0,
                              data.radius, menuName)
            end
        end
        self.NewTimer = GameTimer()
...
function ReleaseEvadeAPI()
    _G.JustEvade = {
        ...
        IsDangerous = function(self, pos)
            return JEvade:IsDangerous(JEvade:To2D(pos))
        end,
        SafePos = function(self) return JEvade:SafePosition() end,
        OnImpossibleDodge = function(self, func)
            JEvade:ImpossibleDodge(func)
        end,
